...
export type Template = { id: string; name: string; description: string; flow: Omit<Flow, 'id' | 'createdAt' | 'updatedAt'>; category: string };
export type RouteState = { path: string; params: Record<string, string>; state?: Record<string, any> };
export type getFlowFn = (id: string) => Promise<
...
- [testing] Split each page packet into small units: parts, the screen body, tests. Every route that the router and tab bar point to must have at least a placeholder module that compiles before anything else, so  (60% · 타 앱 1회 — 맹신 금지)
